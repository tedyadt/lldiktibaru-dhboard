<script>
    $(document).ready(function() {
        const prodiId = '{{$prodiId}}';
        
        @can('create_data_akreditasi_prodi')
        const addButton = {
            text: '+ Add New',
            attr: {
        ...
        const table = $('#table_akreditasi').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route("akreditasi.getByIdperti_prodi", ["id_perti_prodi" => ":prodiId", "byperti_prodi"=>"id_prodi"]) }}'.replace(':prodiId', prodiId),                
            },
            columns: [
                {
...
<script>
    $(document).ready(function() {
        $('#add-new-akreditasi').click(function() {
            var url = '{!! route('akreditasi.create', 'id_prodi=:prodiId&&id_perti=:pertiId') !!}';
            url = url.replace(':prodiId', '{{ $prodiId }}').replace(':pertiId', '{{ $pertiId }}');            
